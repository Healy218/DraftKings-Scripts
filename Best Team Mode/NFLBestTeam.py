...
df = pd.read_csv("../DraftKings Scripts and Stats/NFL Stats/NFLweek1.csv")
...
for pos, count in positions.items():
    prob += sum(player_vars[player, pos]
                for player, p in player_vars.keys() if p == pos) == count

# add constraint to ensure that the selected FLEX isn't already on the team as a RB, WR or TE
# add constraint to ensure that the selected FLEX isn't already on the team as a RB, WR or TE
rb_wr_te_vars = [player_vars[player, pos]
                 for player, pos in player_vars.keys() if pos in ["RB", "WR", "TE"]]
flex_vars = [player_vars[player, pos]
             for player, pos in player_vars.keys() if pos == "FLEX"]
...
prob += sum(player_vars[player_name, pos] * df.loc[df["player"] == player_name,
            "cost"].values[0] for player_name, pos in player_vars.keys()) <= budget

# set objective function
prob += sum(player_vars[player, pos] * df.loc[df["player"] == player,
            "point_value"].values[0] for player, pos in player_vars.keys())
...
if status == 1:
    ...
    for player, pos in player_vars.keys():
        if player_vars[player, pos].varValue == 1.0:
            print(
                f"{player} - {pos} - ${df.loc[df['player'] == player, 'cost'].values[0]} - {df.loc[df['player'] == player, 'point_value'].values[0]} pts")
else:
