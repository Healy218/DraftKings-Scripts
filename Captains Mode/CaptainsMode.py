...
df = pd.read_csv("../DraftKings Scripts and Stats/NBA Stats/NBAgame23.csv")
...
prob += sum(player_vars[(player, "CPT")] for player in df["Name"].unique() if (player, "CPT") in player_vars) == 1

# add constraint for the flex positions
prob += sum(player_vars[(player, "FLEX")] for player in df["Name"].unique() if (player, "FLEX") in player_vars) == 5
...
for player in df["Name"].unique():
    if (player, "CPT") in player_vars and (player, "FLEX") in player_vars:
        prob += player_vars[(player, "CPT")] + player_vars[(player, "FLEX")] <= 1
...
if status == 1:
    ...
    for player, roster_position in player_vars.keys():
        if player_vars[(player, roster_position)].varValue == 1:
            salary = df.loc[(df["Name"] == player) & (df["Roster_Position"] == roster_position), 'Salary'].values[0]
            avg_points = df.loc[(df["Name"] == player) & (df["Roster_Position"] == roster_position), 'AvgPointsPerGame'].values[0]
            teamposition = df.loc[(df["Name"] == player) & (df["Roster_Position"] == roster_position), 'Roster_Position'].values[0]
            total_salary += salary
